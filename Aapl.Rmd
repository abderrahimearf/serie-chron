...
```{r , echo=TRUE,eval="FALSE",message = FALSE,warning = FALSE,fig.show='hold' }
apple_data <- tq_get("AAPL", from = "2000-01-01", to = "2024-01-01")
head(apple_data)
log_return <- diff(log(apple_data$close))
data=ts(apple_data$close, start = c(2000, 1), end = c(2024, 1), frequency = 365)
...
```
## prévision avec modéle GARCH(2,2)
...
## prévision avec modéle E-GARCH(2,2)
...
apple_data_test <- tq_get("AAPL", from = "2024-02-02", to = "2024-10-02")
apple_data_test1 <- ts(diff(log(apple_data_test$close)))
length(apple_data_test1)

plot(VaR_garch, type = "l", col = "red", 
     main = "VaR Prdites (GARCH et EGARCH)",
     xlab = "Priode", ylab = "VaR", lwd = 2)


lines(VaR_egarch, col = "blue", lwd = 2)  



legend("topright", legend = c("VaR GARCH", "VaR EGARCH"),
...
```{r , echo=TRUE,eval="TRUE",message = FALSE,warning = FALSE,fig.show='hold' }
n_test <- length(apple_data_test1)
h <- 160
rolling_forecast <- numeric()

for (start in seq(1, n_test - h + 1, by = h)) {
  data_segment <- apple_data_test1[start:(start + h - 1)]
...
plot(apple_data_test1, type = "l", col = "blue", lwd = 2,
...
realized <- apple_data_test1[7:166]
length(realized)

kupiec_rolling <- VaRTest(alpha = 0.05, actual = realized, VaR = VaR_rolling)
print(kupiec_rolling)

kupiec_rolling1 <- VaRTest(alpha = 0.05, actual = realized, VaR = VaR_rolling1)
...
mse_rolling <- mean((apple_data_test1 - VaR_rolling)^2)#E-GARCH
mse_rolling1 <- mean((apple_data_test1 - VaR_rolling1)^2)#GARCH

cat("MSE pour E-garch :", mse_rolling, "\n")
cat("MSE pour garch  :", mse_rolling1, "\n")


violations <- apple_data_test1 - VaR_rolling
...
WVR <- weighted_violations / length(apple_data_test1)
print(WVR)#E-garch

violations2 <- apple_data_test1 - VaR_rolling1
...
WVR2 <- weighted_violations2 / length(apple_data_test1)
print(WVR2)#garche
...
1. **Précision des modèles (MSE)** :  
   - **E-GARCH** : \( \text{MSE} = 0.001651795 \)  
   - **GARCH** : \( \text{MSE} =  0.001441836 \)  
...
2. **Violations pondérées (WVR)** :  
   - **E-GARCH** : \( \text{WVR} = 0.36601591 \)  
   - **GARCH** : \( \text{WVR} = 0.4749305 \)  
